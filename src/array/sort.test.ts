...
import { createDateSorter, createNumberSorter, createStringSorter } from './sort.ts'
...
Deno.test("createStringSorter", async t => {
  ...
  await t.step('should sort objects by a string key in descending order', () => {
...
Deno.test("createNumberSorter", async t => {
  ...
  await t.step('should sort objects by a number key in descending order', () => {
...
Deno.test("createDateSorter", async t => {
  ...
  const array = [{ date: new Date(2022, 0, 2) }, { date: new Date(2022, 0, 1) }, { date: new Date(2022, 0, 3) }]

  await t.step('should sort objects by a date key in ascending order', () => {
    const result = array.sort(sorter)
    assertEquals(result, [{ date: new Date(2022, 0, 1) }, { date: new Date(2022, 0, 2) }, { date: new Date(2022, 0, 3) }])
  })

  await t.step('should sort objects by a date key in descending order', () => {
    const result = array.sort(createDateSorter('date', false))
    assertEquals(result, [{ date: new Date(2022, 0, 3) }, { date: new Date(2022, 0, 2) }, { date: new Date(2022, 0, 1) }])
  })
})